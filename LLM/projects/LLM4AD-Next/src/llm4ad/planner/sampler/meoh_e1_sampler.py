...
@BaseSampler.register("meoh_e1_sampler")
class MEoHE1Sampler(BaseUnifiedSampler):
    """Generate a distinct new algorithm from multiple parents."""
    ...
    async def sample(self, population: list[Algorithm], generation: int, **kwargs) -> Algorithm:
        ...
        if len(parents) < 2:
            raise ValueError("MEoH E1 requires at least two parents")
