...
class FunSearch:
    ...
    def _sample_evaluate_register(self):
        while (self._max_sample_nums is None) or (self._tot_sample_nums < self._max_sample_nums):
            try:
                ...
                prompt_contents = [prompt.code + 'Do not give additional explanations.' for _ in range(self._samples_per_prompt)]
                ...
                for program, score, eval_time in zip(programs_to_be_eval, scores, times):
                    ...
                    if self._profiler is not None:
                        if type(score) is list:
                            function.score = np.mean(score)
                            function.score_list = score
                        else:
                            function.score = score
                            function.score_list = None
    ...
    def run(self):
        if not self._resume_mode:
            ...
            if type(score) is list:
                self._database.register_function(function=self._function_to_evolve, island_id=None, score=np.mean(score))
            else:
                self._database.register_function(function=self._function_to_evolve, island_id=None, score=score)
            if self._profiler:
                if type(score) is list:
                    self._function_to_evolve.score = np.mean(score)
                    self._function_to_evolve.score_list = score
                else:
                    self._function_to_evolve.score = score
                    self._function_to_evolve.score_list = None
                #self._function_to_evolve.score = score
