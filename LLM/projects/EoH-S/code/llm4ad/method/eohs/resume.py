...
def _resume_pop(log_path: str, pop_size) -> Population:
    ...
    print(f'RESUME EoHS: Generations: {max_gen}.', flush=True)
...
def _resume_pf(log_path: str, pf: EoHSProfiler, template_func):
    ...
    print(f'RESUME EoHS: Sample order: {sample_max_order}.', flush=True)
    pf.__class__._prog_db_order = db_max_order
    # pf.__class__._num_samples = sample_max_order
    for i in tqdm(range(len(funcs)), desc='Resume EoHS Profiler'):  # noqa
...
def resume_EoHS(EoHS: EoHS, path):
    EoHS._resume_mode = True
    pf = EoHS._profiler
    log_path = path
    # resume program database
    pop = _resume_pop(log_path, EoHS._pop_size)
    EoHS._population = pop
    # resume profiler
    template_func = EoHS._function_to_evolve
    _resume_pf(log_path, pf, template_func)
    # resume EoHS
    _, _, sample_max_order, _ = _get_all_samples_and_scores(log_path)
    EoHS._tot_sample_nums = sample_max_order
